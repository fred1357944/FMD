...
- 卡片牆的 ☆ 不再誤開檔（也就不會無端跳出「要不要儲存」）。點星星只改釘選。
- 工具列多一顆 ★：只看已釘選，表格／看板／日曆／卡片共用。
- SLIDEV 與 threads 資料夾都有「卡片封面測試」範例（封面圖在 `card-cover-test.assets/`）。

## 0.5.9 — 2026-09-18

### Library

- 檔名、資料夾名太長時從右邊省略，滑鼠停上去看全名；路徑列會捲到你現在這一層。
- 日曆／卡片的標籤篩選不再把側欄檔案清單清空。

## 0.5.8 — 2026-09-18

### Library

- 檔案側欄左右兩欄中間可以拖，檔名比較長時把右邊拉寬。比例會記住。

## 0.5.7 — 2026-09-18

### Library

- 寫作熱力圖改成一週七格、不再橫向溢出；放在**日曆**上面，點一天會跳到那個月。卡片頁只留封面牆。
- 分頁順序對齊快捷鍵：編輯 ⌘1、表格 ⌘2、看板 ⌘3、日曆 ⌘4、智圖 ⌘5、卡片 ⌘6。
- 智圖色碼（`#81C784` 這類）不再被當成標籤。

### Mindmap

- 點左側大綱會把畫布滾到該節點，不必自己找。

## 0.5.6 — 2026-09-18

### Present

- 同一份簡報再按「立刻開 Slidev」，會把舊的 Vite／node 整組關掉再重開，不再跳到 3031 然後說 port already in use。

## 0.5.5 — 2026-09-18

### Present

- 「立刻開 Slidev」第一張不再是空白：封面 layout 寫進檔頭，不再多一個空的 `---`。
- 資料夾裡現成的 Slidev `slides.md`（例如 `published/` 的建築簡報）可以直接選了按同一顆按鈕，在原地開，不會再轉成 FMD 的 heading 切燈。若 3030 已在播另一份稿，會換下一個埠。

### Library

- 點進子資料夾後，頂部有路徑（可點）和「← 上一層」。右邊空了也不會整片空白，上一層還在。

## 0.5.4 — 2026-09-18

### Library

- 檔案側欄左邊的資料夾可以局部收合，像 Obsidian：▸／▾ 只收那一枝，雙擊資料夾名稱也可以。收合狀態會記住。

## 0.5.3 — 2026-09-18

### Library

- 檔案側欄用小圖標區分資料夾、Markdown、PDF、圖片、程式碼；非 md 檔在右側顯示副檔名（像 PDF）。點 PDF／圖片會用系統預設程式打開，不會塞進編輯器。`package.json` 這類鎖檔不列。

### App

- 終於有自己的 Mac 圖示：奶油紙底上的毛筆 F。
